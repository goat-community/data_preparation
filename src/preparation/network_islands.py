...
class NetworkIslands:
    ...
    def __init__(self, db_config, config_ways):
        self.variable_container_ways = config_ways.preparation
    ...
    def find_connected_network(self):
        ...
        ON w.id = n.id
        WHERE w.class_id NOT IN (SELECT UNNEST(ARRAY{self.variable_container_ways["excluded_class_id_walking"]}))
        AND w.class_id NOT IN (SELECT UNNEST(ARRAY{self.variable_container_ways["excluded_class_id_cycling"]}))
        AND (
            (w.foot NOT IN (SELECT UNNEST(ARRAY{self.variable_container_ways["categories_no_foot"]})) OR foot IS NULL)
            OR
            (w.bicycle NOT IN (SELECT UNNEST(ARRAY{self.variable_container_ways["categories_no_bicycle"]})) OR bicycle IS NULL)
        )
        AND n.id IS NULL 
        LIMIT 1; 
        '''
        ...
        while no_islands_empty == False: 
            ...
            (
                ...
                AND w.class_id NOT IN (SELECT UNNEST(ARRAY{self.variable_container_ways["excluded_class_id_walking"]}))
                AND w.class_id NOT IN (SELECT UNNEST(ARRAY{self.variable_container_ways["excluded_class_id_cycling"]}))
                AND (
                    (w.foot NOT IN (SELECT UNNEST(ARRAY{self.variable_container_ways["categories_no_foot"]})) OR foot IS NULL)
                    OR
    ...
    def find_network_islands(self):
        ...
        sql_classify_network_islands =  f"""
            ...
            AND w.class_id NOT IN (SELECT UNNEST(ARRAY{self.variable_container_ways["excluded_class_id_walking"]}))
            AND w.class_id NOT IN (SELECT UNNEST(ARRAY{self.variable_container_ways["excluded_class_id_cycling"]}))
            AND (w.foot NOT IN (SELECT UNNEST(ARRAY{self.variable_container_ways["categories_no_foot"]})) OR foot IS NULL); 
