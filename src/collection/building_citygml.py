...
class CityGMLCollection:
    ...
    def insert_buildings(self, buildings: list):
        ...
        for i in range(0, len(buildings), self.insert_chunk_size):
            ...
            buildings_to_insert = [
                (b[0], b[1], b[2], b[3], b[4]) for b in buildings_to_insert
            ]
            buildings_to_insert = str(buildings_to_insert).replace("None", "NULL")
            self.db.perform(
                f"""INSERT INTO basic.building (id, roof_levels, building_levels, building_type, geom) VALUES {buildings_to_insert[1:-1]}"""
            )
            print(f"Inserted {i} buildings into the database.")
    
    #TODO: Put this in utils. And add logic if table does not exist
    def check_table_exists(self):
        ...
        if check_if_exists[0][0] == True:
            ...
            raise NameError(
                "Building table does not exist. Please create the table first."
            )
